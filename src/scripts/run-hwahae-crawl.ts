...
const logger = createCrawlLogger("hwahae");
...
async function main() {
  ...
  logger.event("info", "main_start", {
    ...
    dryRun: opts.dryRun,
    themes: opts.themes ?? VALID_THEMES,
    maxLeavesPerTheme: opts.maxLeavesPerTheme ?? null,
  });
  ...
  const outcome = await crawlHwahaeRankings({
    ...
    log: (level, msg) => {
      logger.event(level, "crawl_progress", { message: msg });
  ...
  logger.event("info", "crawl_summary", {
    themes: outcome.themes.length,
    categoryNodes: outcome.categoryNodes.length,
    products: outcome.products.length,
    brandRanks: outcome.brandRanks.length,
  ...
  if (outcome.errors.length > 0) {
    logger.event("warn", "crawl_errors_present", {
      count: outcome.errors.length,
      sample: outcome.errors.slice(0, 5),
    });
  ...
  if (outcome.products.length === 0) {
    logger.event("warn", "empty_result_skip");
    console.log(`\n⚠️ 상품 0건 — DB 저장 skip`);
    return;
  }
  ...
  console.log(`\n💾 DB 저장 시작...`);
  logger.event("info", "db_persist_start", {
    productCount: outcome.products.length,
    brandRanks: outcome.brandRanks.length,
  });
  ...
  logger.event("info", "db_persist_complete", {
    ...summary,
  });
